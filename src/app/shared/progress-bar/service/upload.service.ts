...
import { HttpParamEnum } from '../../enums/http-params.enums';
...
export class UploadService {
  ...
  public upload(files: Set<File>, vendorId: string,
    companyInfo: string, transactionStartDate: string, transactionEndDate: string): { [key: string]: {progress: Observable<number>} } {
    const status: { [key: string]: {progress: Observable<number>} } = {};

    files.forEach((file: File) => {
      const formData: FormData = new FormData();
      const  params: HttpParams = this.buildUploadParams(vendorId, companyInfo, transactionStartDate, transactionEndDate);
      ...
      this._http.request(req).subscribe((event: any) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          const percentDone = Math.round(100 * event.load / event.total);
  ...
  public buildUploadParams(vendorId: string, companyInfo: string, transactionStartDate: string, transactionEndDate: string): HttpParams {
    return new HttpParams().set(HttpParamEnum.vendorId, vendorId)
    .set(HttpParamEnum.companyInfo, companyInfo)
    .set(HttpParamEnum.transactionStartDate, transactionStartDate)
    .set(HttpParamEnum.transactionEndDate, transactionEndDate);
  }
}
